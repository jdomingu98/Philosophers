...
static int init_mtx(t_data *data)
{
	...
	if (pthread_mutex_init(&data->eaten_mtx, NULL))
		return (free_mtx(data, ""), 0);
	if (pthread_mutex_init(&data->philo_death_mtx, NULL))
		return (free_mtx(data, "death"), 0);
	while (i < data->nphilos)
	{
		if (pthread_mutex_init(&data->philos[i].fork_mtx, NULL))
			return (free_philo_mtx(data, i, "f"), free_mtx(data, "all"), 0);
		if (pthread_mutex_init(&data->philos[i].meals_mtx, NULL))
			return (free_philo_mtx(data, i, "m"), free_mtx(data, "all"), 0);
		i++;
	}
	return (1);
}

static int	get_actual_time(void) // mirar que hacia gettimeofday
{
	struct timeval time;
...
static void print_status(t_philo *philo, char *str)
{
	...
	printf("[%d]	%d %s\n", get_actual_time() - data->t0, philo->id, str);
...
static void	*routine(void *philo_data)
{
	...
	while (!meals_eaten(data) && !philo_died(data))
	{
		...
		pthread_mutex_lock(&data->philos[philo->id % data->nphilos].fork);
		print_status(philo, "has taken a fork");
		...
		pthread_mutex_unlock(&data->philos[philo->id % data->nphilos].fork);
...
static int	check_death(t_philo philo)
{
	...
	if (philo->last_meal + philo->data->time_die < get_actual_time())
	{
		...
		pthread_mutex_lock(&philo->data->philo_death_mtx); //innecesario creo
